...
- All new identity strings (name, binary, MCPB, marketplace, domain) route through one config module so the loupe→bodkin swap (Phase 6) is one change; keep `bodkin` as the working name, contingent on the [ADR 0004](../adr/0004-rename-loupe-to-bodkin.md) confirmations.
...
- [ ] Rebuild each on shadcn-svelte primitives restyled to the bodkin palette; parity with current features.
...
- [ ] Install script (`bodkin.sh/install[.ps1]`) hosted on Cloudflare; GitHub Actions tag-triggered matrix builds 3 binaries + 3 MCPBs + checksums + Release; Homebrew/Scoop/winget manifests; `bodkin update` (install-aware self-update).
- [ ] Update Claude Code + Codex plugin docs to the install-script flow (drop the hidden clone requirement).

### Phase 6 — Rebrand swap
- [ ] Flip identity strings loupe→bodkin via the one config module: `package.json`, binary, MCPB `name`/`display_name`, marketplace manifests, homepage/repo URLs; GitHub repo rename + redirect. (Gate on ADR 0004 confirmations.)

### Phase 7 — Docs replatform
- [ ] Astro Starlight site (Getting started / Guides / Reference) in the bodkin brand, hosted on Cloudflare Pages; retire `site/` + `.github/workflows/pages.yml`.
