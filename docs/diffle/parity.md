...
A Phase 0 audit of the retired Preact client folded the behaviors above into this checklist. It also surfaced three claims the retired client did **not** actually satisfy. These are now **resolved as firm diffle requirements** — the rewrite honors the contract for all three; they are not optional and each is built in the slice that owns its surface:

- **The markdown preview is sanitized.** The retired client rendered `marked@12` output with no sanitizer, so raw HTML in a `.md` rendered unescaped. diffle sanitizes rendered markdown before insertion (allowlist — no scripts, event-handler attributes, or dangerous URL schemes), preserving review-relevant formatting and relative-image resolution through `/api/raw`. Decision recorded in [ADR 0007](../adr/0007-sanitize-markdown-preview.md); implemented in the markdown slice (Phase 2).
- **Dialogs trap focus and restore it.** diffle dialogs (help, What's New, compile/feedback) trap Tab within the dialog, set initial focus on open, mark the background inert, close on Escape (only the active overlay), and return focus to the trigger on close. Built with the overlay/dialog primitives (Phase 2) and verified in the accessibility pass (Phase 3).
- **Empty and no-diff states are real states.** An added file with no content renders a deliberate empty-file state, and a review whose diff has no files renders an explicit "no changes" state — never a blank pane. Built in the diff/file slices (Phase 2).
