...
export async function handleGetRadar(ctx: ServerContext, _url: URL): Promise<Response> {
  return json(await analyzeRadar(ctx.diff, false, process.env, fetch, true, radarContext(ctx), readUserState().radarMode));
}
...
export async function handlePostRadar(ctx: ServerContext, req: Request): Promise<Response> {
  ...
  if (!task) {
    task = analyzeRadar(ctx.diff, refresh, process.env, fetch, false, radarContext(ctx), readUserState().radarMode);
...
export function handleGetRadarPacket(ctx: ServerContext, url: URL): Response {
  ...
  try {
    const mode = readUserState().radarMode;
    const packet = radarPacket(ctx.diff, id, process.env, radarContext(ctx), mode);
    const config = providerFromEnv(process.env, mode);
